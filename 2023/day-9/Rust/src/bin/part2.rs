...
fn add_values(values: &mut Vec<Vec<i32>>) {
    values.first_mut().unwrap().push(0);

    for i in (0..values.len() - 2).rev() {
        let next_last_value = values[i + 1].first().copied().unwrap();
        let last_value = values[i].first().copied().unwrap();
        values[i].insert(0,last_value - next_last_value);
    }
}
...
pub fn main() -> io::Result<()> {
    ...
    for line in lines {
        ...
        sum += values.first().unwrap().first().unwrap();
    }
